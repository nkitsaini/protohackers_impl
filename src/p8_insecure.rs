...
impl Cipher {
    ...
    fn new(cipher: Vec<u8>) -> anyhow::Result<Self> {
        Ok(Self { operations: CipherOp::parse_all(cipher)? })
    }
...
impl ToyInfo {
    pub fn new(content: &str) -> anyhow::Result<Self> {
        Ok(Self {
            ...
            count: regex::Regex::new(r#"(^\d+)x"#).unwrap().captures(content).context("no number at start")?.get(1).unwrap().as_str().parse()?,
        })
    }
}
...
async fn handle_client(sock: tokio::net::TcpStream) -> anyhow::Result<()> {
    ...
    loop {
		while let Some(i) = prev_buf.find('\n') {
			...
			let toys = line.split(',').into_iter().map(|x| ToyInfo::new(x)).collect::<anyhow::Result<Vec<_>>>()?;
			let best_toy = toys
				.iter()
...
mod tests {
    ...
    async fn p8_test_client() -> anyhow::Result<()> {
        ...
        let msg1 = vec![0xf2, 0x20, 0xba, 0x44, 0x18, 0x84, 0xba, 0xaa, 0xd0, 0x26, 0x44, 0xa4, 0xa8, 0x7e];
        write_half1.write_all(&msg1).await?;
