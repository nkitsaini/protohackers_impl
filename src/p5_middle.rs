...
const ADDRESS_REGEX: Lazy<Regex> = Lazy::new(|| {Regex::new(
	r#"(^| )7[a-zA-Z0-9]{25,34}($| |\n)"#
).unwrap()});
...
const CHAT_SERVER_SOCK_ADDR: Lazy<SocketAddr> = Lazy::new(|| {
	CHAT_SERVER_ADDR.to_socket_addrs().unwrap().filter(|x| x.is_ipv4()).next().unwrap()
});

fn replace_address(val: &str) -> String {
	let val = val.replace(' ', "  ");
	let val = ADDRESS_REGEX.replace_all(&val, TONY_ADDRESS_REP).to_string();
	val.replace("  ", " ")
}
...
impl LinedBufReader {
	pub fn new(val: tokio::io::BufReader<tokio::net::tcp::OwnedReadHalf>) -> Self {
		Self{ reader: val, prev_part: Default::default()}
	}
	pub async fn get_next_line(&mut self) -> Option<String> {
		loop {
			...
			self.prev_part.push_str(&String::from_utf8(buf[..r].to_vec()).ok()?);
		}
	}
}
...
async fn handle_user(
    ...
    let s_stream = tokio::net::TcpSocket::new_v4()?.connect(dbg!(*CHAT_SERVER_SOCK_ADDR)).await?;
